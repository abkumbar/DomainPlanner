...
class PlanningGraph():
        ...
        for action in self.all_actions:
            if PgNode_a(action).prenodes.issubset(self.s_levels[level]):
                for sNode in self.s_levels[level]:
                    if sNode in PgNode_a(action).prenodes:
                        sNode.children.add(PgNode_a(action))
                        PgNode_a(action).parents.add(sNode)
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for n1 in node_a1.action.effect_add:
            for n2 in node_a2.action.precond_neg:
                if n1==n2:
                    return True
        ...
        for n1 in node_a1.action.effect_rem:
            for n2 in node_a2.action.precond_pos:
                if n1==n2:
                    return True
        
        for n1 in node_a1.action.precond_pos:
            for n2 in node_a2.action.effect_rem:
                if n1==n2:
                    return True
        return False
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for n1 in node_s1.parents:
            for n2 in node_s2.parents:
                if not (n1.is_mutex(n2) or n2.is_mutex(n1)):
                    return False
        return True
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            for i,state in enumerate(self.s_levels,1):
                for node in state:
